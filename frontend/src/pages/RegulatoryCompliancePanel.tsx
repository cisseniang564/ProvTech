...
const generateIFRS17FromActuarialData = (result: any, extendedKPI: any): IFRS17RealData | null => {
  ...
  // Utiliser les vraies données des méthodes actuarielles
  const methods = result?.methods || [];
  const ultimateTotal = result?.summary?.bestEstimate || methods.reduce((sum: number, m: any) => sum + (m.ultimate || 0), 0);
  const paidToDate = methods.reduce((sum: number, m: any) => sum + (m.paid_to_date || 0), 0) / Math.max(methods.length, 1);
  const outstandingReserves = methods.reduce((sum: number, m: any) => sum + (m.reserves || 0), 0) / Math.max(methods.length, 1);
  
  // Calcul CSM basé sur les données réelles et la marge estimée
  const profitMarginRate = 0.08 + (extendedKPI?.dataQualityScore || 75) / 1000; // Marge variable selon qualité
  const csmBalance = outstandingReserves * profitMarginRate;
  
  // Estimation des primes basée sur les données actuarielles réelles
  let estimatedPremiums = 0;
  if (extendedKPI?.ultimateLossRatio && extendedKPI.ultimateLossRatio > 0) {
    estimatedPremiums = ultimateTotal / (extendedKPI.ultimateLossRatio / 100);
  } else if (extendedKPI?.lossRatio && extendedKPI.lossRatio > 0) {
    estimatedPremiums = paidToDate / (extendedKPI.lossRatio / 100);
  } else {
    // Fallback: estimation basée sur les réserves et un multiple standard
    estimatedPremiums = ultimateTotal / 0.75; // Assume 75% loss ratio
  }
  
  // Générer les mouvements CSM avec les vraies données temporelles
  const movements: IFRS17RealData['contractualServiceMargin']['movements'] = [];
  let balance = csmBalance * 1.15; // Balance d'ouverture estimée
  
  // Calculer le taux d'intérêt réel basé sur la qualité des données
  const interestRate = 0.01 + (extendedKPI?.dataQualityScore || 75) / 10000;
  const serviceReleaseRate = Math.max(0.02, Math.min(0.05, (extendedKPI?.maturityRatio || 70) / 2000));
  
  for (let i = 0; i < 4; i++) {
    const quarterDate = new Date();
    quarterDate.setMonth(quarterDate.getMonth() - (3 - i) * 3);
    
    const interestAccretion = balance * interestRate;
    const serviceRelease = -balance * serviceReleaseRate;
    // Experience adjustments basés sur la convergence des méthodes
    const experienceAdj = balance * ((extendedKPI?.convergenceIndex || 85) - 85) / 10000;
    const unlockingAdj = i === 1 ? -balance * 0.005 : 0;
    
    const openingBalance = balance;
    balance = balance + interestAccretion + serviceRelease + experienceAdj + unlockingAdj;
    
    // Coverage units basées sur les données réelles de primes
    const coverageUnits = Math.max(10000, estimatedPremiums / 1000 * (1 - i * 0.15));
    ...
    movements.push({
      ...
      confidenceInterval: {
        lower: balance * (0.85 + (extendedKPI?.dataQualityScore || 75) / 1000),
        upper: balance * (1.15 - (extendedKPI?.dataQualityScore || 75) / 1000)
      }
    });
  }
  
  // Risk Adjustment calculé avec les données de volatilité réelles
  const riskAdjustmentRate = 0.06 + (extendedKPI?.coefficientOfVariation || 15) / 1000;
  const riskAdjustmentTotal = outstandingReserves * riskAdjustmentRate;
  const nonFinancialRisks = riskAdjustmentTotal * 1.2;
  const diversificationBenefit = -riskAdjustmentTotal * Math.max(0.1, (extendedKPI?.diversificationBenefit || 15) / 100);
  
  // Revenue calculation basée sur les primes estimées
  const insuranceRevenue = estimatedPremiums;
  const serviceExpenses = insuranceRevenue * ((extendedKPI?.combinedRatio || 95) / 100);
  const netFinancialResult = csmBalance * 0.04; // 4% return on CSM
  ...
  return {
    ...
    riskAdjustment: {
      ...
      confidenceLevel: Math.max(70, Math.min(85, extendedKPI?.dataQualityScore || 75)),
    ...
    disclosureTables: {
      ...
      attributionAnalysis: [
        { component: 'Insurance Revenue', amount: insuranceRevenue, variance: (extendedKPI?.convergenceIndex - 85) / 1000 || 0 },
        { component: 'Service Expenses', amount: -serviceExpenses, variance: -(extendedKPI?.coefficientOfVariation || 15) / 1000 },
        { component: 'Financial Result', amount: netFinancialResult, variance: (extendedKPI?.dataQualityScore - 80) / 2000 || 0 }
      ]
    },
    validationStatus: extendedKPI?.dataQualityScore > 90 ? 'valid' : 
                     extendedKPI?.dataQualityScore > 75 ? 'warning' : 'error',
...
const generateSolvency2FromActuarialData = (result: any, extendedKPI: any): SolvencyIIRealData | null => {
  ...
  const methods = result?.methods || [];
  const ultimateTotal = result?.summary?.bestEstimate || methods.reduce((sum: number, m: any) => sum + (m.ultimate || 0), 0);
  const reserves = methods.reduce((sum: number, m: any) => sum + (m.reserves || 0), 0) / Math.max(methods.length, 1);
  const paidToDate = methods.reduce((sum: number, m: any) => sum + (m.paid_to_date || 0), 0) / Math.max(methods.length, 1);
  
  // Estimation des primes basée sur les ratios réels
  let estimatedPremiums = 0;
  if (extendedKPI?.ultimateLossRatio && extendedKPI.ultimateLossRatio > 0) {
    estimatedPremiums = ultimateTotal / (extendedKPI.ultimateLossRatio / 100);
  } else if (extendedKPI?.lossRatio && extendedKPI.lossRatio > 0) {
    ...
    estimatedPremiums = ultimateTotal / 0.75; // Fallback
  }
  
  // Calcul des risques basé sur les données réelles et la volatilité
  const volatilityMultiplier = 1 + ((extendedKPI?.coefficientOfVariation || 15) - 15) / 100;
  const businessLineMultiplier = result?.metadata?.businessLine === 'automobile' ? 0.9 : 
                                result?.metadata?.businessLine === 'dab' ? 1.1 : 1.0;
  
  const marketRisk = (reserves * 0.18 + estimatedPremiums * 0.08) * volatilityMultiplier;
  const underwritingRisk = (reserves * 0.15 + estimatedPremiums * 0.12) * businessLineMultiplier;
  const counterpartyRisk = reserves * 0.04 * Math.max(0.8, (extendedKPI?.dataQualityScore || 75) / 100);
  const operationalRisk = (estimatedPremiums + reserves) * 0.03;
  const intangibleRisk = reserves * 0.025;
  
  // Matrice de corrélation ajustée selon la ligne d'activité
  const baseCorrelationMatrix = [
  ...
  // Ajustement de la corrélation selon la convergence des méthodes
  const correlationAdjustment = (extendedKPI?.methodAgreement || 90) / 100;
  const correlationMatrix = baseCorrelationMatrix.map(row => 
    row.map(val => val === 1.0 ? 1.0 : val * correlationAdjustment)
  );
  
  // Calcul SCR avec corrélations ajustées
  const riskVector = [marketRisk, underwritingRisk, counterpartyRisk, operationalRisk, intangibleRisk];
  let basicSCR = 0;
  
  // Calcul matriciel approximatif du SCR avec corrélations
  for (let i = 0; i < riskVector.length; i++) {
    for (let j = 0; j < riskVector.length; j++) {
      basicSCR += riskVector[i] * riskVector[j] * correlationMatrix[i][j];
    }
  }
  basicSCR = Math.sqrt(basicSCR);
  
  // Diversification benefit basé sur les données réelles
  const diversificationRate = Math.min(0.3, (extendedKPI?.diversificationBenefit || 15) / 100);
  const diversificationBenefit = -basicSCR * diversificationRate;
  const totalSCR = basicSCR + diversificationBenefit;
  
  // Calcul MCR avec les facteurs réels
  const premiumFactor = estimatedPremiums * 0.08; // 8% des primes
  const reserveFactor = reserves * 0.10; // 10% des réserves
  const linearMCR = Math.max(premiumFactor + reserveFactor, totalSCR * 0.25);
  const absoluteFloor = 3_700_000;
  const cappedMCR = Math.min(linearMCR, totalSCR * 0.45);
  const finalMCR = Math.max(cappedMCR, absoluteFloor);
  
  // Own Funds calculés selon la qualité des données
  const baseOwnFunds = totalSCR * 1.5; // Base de 150%
  const qualityAdjustment = 1 + (extendedKPI?.dataQualityScore - 80) / 1000;
  const solvencyRatio = extendedKPI?.combinedRatio ? 
    Math.max(100, 200 - extendedKPI.combinedRatio) : 158;
  const ownFundsTotal = totalSCR * (solvencyRatio / 100) * qualityAdjustment;
  
  // QRT Status basé sur la qualité des données et la compliance
  const qrtStatus = [
    {
      templateId: 'S.02.01',
      status: extendedKPI?.dataQualityScore > 90 ? 'validated' as const : 'submitted' as const,
      validationErrors: extendedKPI?.dataQualityScore < 80 ? ['Data quality below threshold'] : [],
  ...
  // Stress tests basés sur les données réelles
  const baselineRatio = (ownFundsTotal / totalSCR) * 100;
  const stressTests = [
    { 
      scenario: 'Choc de marché -20%', 
      impact: -0.2 * (marketRisk / totalSCR), 
      solvencyRatio: baselineRatio * (1 - 0.2 * (marketRisk / totalSCR)), 
      passed: baselineRatio * (1 - 0.2 * (marketRisk / totalSCR)) >= 100 
    },
    { 
      scenario: 'Hausse sinistralité +30%', 
      impact: -0.3 * (underwritingRisk / totalSCR), 
      solvencyRatio: baselineRatio * (1 - 0.3 * (underwritingRisk / totalSCR)), 
      passed: baselineRatio * (1 - 0.3 * (underwritingRisk / totalSCR)) >= 100 
    },
    { 
      scenario: 'Pandémie', 
      impact: -0.25, 
      solvencyRatio: baselineRatio * 0.75, 
      passed: baselineRatio * 0.75 >= 100 
    },
    { 
      scenario: 'Catastrophe naturelle', 
      impact: -0.18 * businessLineMultiplier, 
      solvencyRatio: baselineRatio * (1 - 0.18 * businessLineMultiplier), 
      passed: baselineRatio * (1 - 0.18 * businessLineMultiplier) >= 100 
    }
  ];
  ...
  return {
    scrCalculation: {
      ...
      correlationMatrix,
      subModules: [
        { name: 'Interest Rate Risk', value: marketRisk * 0.35, parentModule: 'Market Risk' },
        { name: 'Equity Risk', value: marketRisk * 0.40, parentModule: 'Market Risk' },
        { name: 'Property Risk', value: marketRisk * 0.15, parentModule: 'Market Risk' },
        { name: 'FX Risk', value: marketRisk * 0.10, parentModule: 'Market Risk' },
        { name: 'Premium Risk', value: underwritingRisk * 0.65, parentModule: 'Underwriting Risk' },
        { name: 'Reserve Risk', value: underwritingRisk * 0.35, parentModule: 'Underwriting Risk' },
        { name: 'Counterparty Default', value: counterpartyRisk * 0.80, parentModule: 'Counterparty Risk' },
        { name: 'Reinsurance Risk', value: counterpartyRisk * 0.20, parentModule: 'Counterparty Risk' }
      ]
    },
    mcrCalculation: {
      linearMCR,
    ...
    solvencyRatios: {
      scrCoverage: (ownFundsTotal / totalSCR) * 100,
      mcrCoverage: (ownFundsTotal / finalMCR) * 100,
      trend: extendedKPI?.dataQualityScore > 85 ? 'improving' : 
             extendedKPI?.dataQualityScore > 70 ? 'stable' : 'deteriorating'
    },
    qrtStatus,
    lastCalculated: new Date().toISOString(),
    stressTests
...
const generateDynamicComplianceChecks = (
  ...
  // Données réelles des méthodes actuarielles pour les checks
  const methods = result?.methods || [];
  const ultimateTotal = result?.summary?.bestEstimate || methods.reduce((sum: number, m: any) => sum + (m.ultimate || 0), 0);
  const avgReserves = methods.reduce((sum: number, m: any) => sum + (m.reserves || 0), 0) / Math.max(methods.length, 1);
  const avgPaidToDate = methods.reduce((sum: number, m: any) => sum + (m.paid_to_date || 0), 0) / Math.max(methods.length, 1);
  
  // Estimation des primes basée sur les ratios réels (cohérent avec les fonctions de génération)
  let estimatedPremiums = 0;
  if (extendedKPI?.ultimateLossRatio && extendedKPI.ultimateLossRatio > 0) {
    estimatedPremiums = ultimateTotal / (extendedKPI.ultimateLossRatio / 100);
  } else if (extendedKPI?.lossRatio && extendedKPI.lossRatio > 0) {
    estimatedPremiums = avgPaidToDate / (extendedKPI.lossRatio / 100);
  } else {
    estimatedPremiums = ultimateTotal / 0.75;
  }
  ...
  if (ifrs17Data) {
    ...
    checks.push({
      ...
      requirement: '2-5% par trimestre',
      actualValue: `${(latestMovement?.releaseRate * 100).toFixed(2)}%`,
      threshold: 5,
      criticality: 'medium'
    });

    // Nouveau check : cohérence P&L IFRS 17
    const calculatedProfit = ifrs17Data.disclosureTables.insuranceRevenue - ifrs17Data.disclosureTables.insuranceServiceExpenses + ifrs17Data.disclosureTables.netFinancialResult;
    checks.push({
      id: 'ifrs17-pl-coherence',
      category: 'IFRS17',
      name: 'P&L Coherence Check',
      description: 'Cohérence arithmétique du P&L IFRS 17',
      status: Math.abs(calculatedProfit - ifrs17Data.disclosureTables.profitBeforeTax) < 10000 ? 'compliant' : 'non-compliant',
      requirement: 'Revenue - Expenses + Financial = Profit',
      actualValue: `Écart: ${Math.abs(calculatedProfit - ifrs17Data.disclosureTables.profitBeforeTax).toFixed(0)}€`,
      threshold: 10000,
      criticality: 'high'
    });

    // Nouveau check : validation CSM vs réserves
    const csmToReservesRatio = (ifrs17Data.contractualServiceMargin.currentBalance / avgReserves) * 100;
    checks.push({
      id: 'ifrs17-csm-reserves-ratio',
      category: 'IFRS17',
      name: 'CSM to Reserves Ratio',
      description: 'Ratio CSM vs réserves dans les bornes raisonnables',
      status: csmToReservesRatio < 5 || csmToReservesRatio > 20 ? 'warning' : 'compliant',
      requirement: '5-20% des réserves',
      actualValue: `${csmToReservesRatio.toFixed(1)}%`,
      threshold: 20,
      criticality: 'medium'
    });
  }
  ...
  if (solvencyData) {
    ...
    // Nouveau check : MCR linéaire vs seuils
    const mcrLinearToSCR = (solvencyData.mcrCalculation.linearMCR / solvencyData.scrCalculation.totalSCR) * 100;
    checks.push({
      id: 'sii-mcr-bounds',
      category: 'SII',
      name: 'MCR Linear Formula Bounds',
      description: 'MCR linéaire dans les bornes réglementaires (25%-45% SCR)',
      status: mcrLinearToSCR < 25 || mcrLinearToSCR > 45 ? 'warning' : 'compliant',
      requirement: '25-45% du SCR',
      actualValue: `${mcrLinearToSCR.toFixed(1)}% du SCR`,
      threshold: 45,
      criticality: 'medium'
    });

    // Nouveau check : validation primes estimées
    if (estimatedPremiums > 0) {
      const premiumsToReservesRatio = avgReserves / estimatedPremiums;
      checks.push({
        id: 'sii-reserves-to-premiums',
        category: 'SII',
        name: 'Reserves to Premiums Ratio',
        description: 'Ratio réserves/primes dans les bornes sectorielles',
        status: premiumsToReservesRatio > 0.8 ? 'warning' : 'compliant',
        requirement: '< 80% (run-off business)',
        actualValue: `${(premiumsToReservesRatio * 100).toFixed(1)}%`,
        threshold: 80,
        criticality: 'medium'
      });
    }
  ...
  // Checks basés sur les extendedKPI enrichis
  ...
  // Nouveau check : cohérence données actuarielles
  if (methods.length > 1) {
    const ultimates = methods.map((m: any) => m.ultimate).filter((u: number) => u > 0);
    const coeffVar = ultimates.length > 1 ? 
      (Math.sqrt(ultimates.reduce((acc: number, val: number) => acc + Math.pow(val - ultimates.reduce((a: number, b: number) => a + b, 0) / ultimates.length, 2), 0) / ultimates.length) / 
       (ultimates.reduce((a: number, b: number) => a + b, 0) / ultimates.length)) * 100 : 0;
    
    checks.push({
      id: 'actuarial-convergence',
      category: 'ACTUARIAL',
      name: 'Methods Convergence',
      description: 'Convergence entre les méthodes actuarielles',
      status: coeffVar > 15 ? 'warning' : coeffVar > 25 ? 'non-compliant' : 'compliant',
      requirement: '< 15% coefficient variation',
      actualValue: `${coeffVar.toFixed(1)}% CV`,
      threshold: 15,
      criticality: 'medium'
    });
  }

  // Nouveau check : qualité des données d'entrée
  if (result?.metadata?.dataPoints) {
    checks.push({
      id: 'data-quality-points',
      category: 'DATA',
      name: 'Data Points Sufficiency',
      description: 'Nombre suffisant de points de données pour les calculs',
      status: result.metadata.dataPoints < 15 ? 'warning' : 'compliant',
      requirement: '≥ 15 points de données',
      actualValue: `${result.metadata.dataPoints} points`,
      threshold: 15,
      criticality: 'low'
    });
  }
